...
export default function WordDetailPage() {
    ...
    const fetchWord = useCallback(async () => {
        try {
            const data = await apiFetch<NotionWord>(`/api/words/${id}`)
            setWord(data)
        } catch (err: any) {
    ...
    const isPortugueseVerb =
        word?.language === 'Portuguese' && word?.typo === 'Verbo'
    const hasVerbForms =
        isPortugueseVerb &&
        (word.voce || word.eleEla || word.elesElas || word.nos)
                            ...
                            <div className="bg-muted rounded-md p-3">
                                <table className="w-full">
                                    <tbody className="space-y-2">
                                        {word.voce && (
                                            <tr>
                                                <td className="text-sm font-medium text-muted-foreground pr-4 py-1">
                                                    Você
                                                </td>
